...
class VoiceActivityDetector {
  ...
  async start(): Promise<void> {
    ...
    try {
      // All three processing constraints set explicitly rather than left to
      // implicit browser defaults. echoCancellation is off deliberately:
      // its job is suppressing mic input correlated with recently-played
      // output, and this app's speak-then-listen cycle (TTS plays, then the
      // mic reopens ~250ms later, on what's very likely the same device) is
      // exactly the condition that stresses it hardest -- real captured
      // logs showed speech-attempt levels collapsing from comfortably above
      // threshold on turn 1 to well below it on turn 2+, while the ambient
      // noise floor stayed flat, consistent with voice itself being
      // attenuated rather than a generic mic/gain problem. autoGainControl
      // and noiseSuppression stay explicitly ON (not left undefined) --
      // disabling all three together was already tried and reverted after
      // real evidence showed it made a fresh conversation's first turn
      // measurably worse (levels of 0.0-1.0 even from a cold start).
      this.stream = await navigator.mediaDevices.getUserMedia({
        audio: { echoCancellation: false, autoGainControl: true, noiseSuppression: true },
      });
    } catch (err) {
