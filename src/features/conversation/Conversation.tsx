...
export function Conversation({ profile, initialMessage, onExit }: ConversationProps) {
  ...
  // Sends the quick-action/free-text message that opened this screen,
  // exactly once. A proper effect (not a check in the render body) so it
  // runs on mount rather than as a side effect of rendering — correct
  // under React.StrictMode's double-render in dev, and not fragile if
  // React ever renders this component more than once for other reasons.
  useEffect(() => {
    if (initialMessage && !started.current) {
      started.current = true
      void send(initialMessage)
    }
    // Intentionally run once on mount only — send() and initialMessage
    // are stable for the lifetime of a single conversation screen.
